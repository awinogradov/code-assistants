---
name: autopilot-resolve-issue-context
description: >-
  Fetch issue context from GitHub (gh) or Linear (bundled GraphQL helper keyed
  by LINEAR_API_KEY) and optionally auto-assign the current user (idempotent,
  opt-in via caller flag). Use when commands need structured issue data without
  polluting parent context.
---
> Derived from the autopilot `resolve-issue-context` subagent. Where subagents are unavailable, run this task inline and treat its structured output block as the result handed back to the invoking workflow.
...
<!-- Canonical self-assign logic. Mirrored in [skills/branch:create/SKILL.md Phase 2](../autopilot-branch-create/SKILL.md#phase-2-fetch-github-issue). Keep in sync. -->
