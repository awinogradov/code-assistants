---
name: autopilot-analyze-staged-changes
description: >-
  Categorize staged files and assess commit strategy. Use when commits:create
  needs pre-computed analysis without polluting parent conversation.
---
> Derived from the autopilot `analyze-staged-changes` subagent. Where subagents are unavailable, run this task inline and treat its structured output block as the result handed back to the invoking workflow.
