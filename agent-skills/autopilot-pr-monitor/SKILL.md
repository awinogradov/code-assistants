---
name: autopilot-pr-monitor
description: >-
  Monitor a PR for review approval and CI check status, blocking until approved
  with all checks passing. Fixes CI failures and resolves review feedback. Use
  when waiting for PR approval.
---
...
- `prompt`: "Invoke `autopilot-pr-monitor`. Monitor the PR in background mode — poll for review approval AND CI check status but do NOT invoke pr:resolve interactively or fix CI checks. Instead, return immediately when changes are requested or checks fail with a structured summary."
...
- **Do NOT invoke** `autopilot-pr-resolve` — the user is not available for interaction
...
2. Invoke `autopilot-pr-resolve` to evaluate unresolved suggestions and nitpicks. The skill will exit early if no actionable comments remain. For each suggestion:
...
4. After fixes, commit via `autopilot-commits-create` and push:
   ```bash
   git push
   ```
