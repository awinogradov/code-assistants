---
name: autopilot-todo-cleanup
description: >-
  Scan codebase for TODO/FIXME comments, verify actuality, create GitHub issues,
  and update links
---
...
3. **For each TODO or group of TODOs:**
   ...
   - **Linear:** the Linear MCP `save_issue` tool with `{ "title": "<title>", "team": "<team>", "description": "<body>" }` (the `team` resolved in step 1 above)
     ...
     **Linear MCP access:** Read [`linear-mcp-access.md`](../autopilot-shared-rules/references/linear-mcp-access.md) and apply its tool-resolution rule, using the bare tool names `save_issue`.
