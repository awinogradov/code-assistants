---
name: autopilot-issue-run
description: >-
  List recent open GitHub or Linear issues, pick one, and start autopilot on it
  via the run skill. Use to go from browsing issues to a running autopilot
  session in one step.
---
...
**Decision points.** Every user choice below uses AskUserQuestion (single-select). Read [`askuserquestion-format.md`](../autopilot-shared-rules/references/askuserquestion-format.md) once and apply it to every `question` you compose.
...
**Linear MCP access:** Read [`linear-mcp-access.md`](../autopilot-shared-rules/references/linear-mcp-access.md) and apply its tool-resolution rule, using the bare tool names `list_issues`.
