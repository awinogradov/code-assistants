---
name: autopilot-commits-restructure
...
It does NOT update the pull request: its [Phase 5](../autopilot-commits-create/SKILL.md#phase-5-update-pr) defers to this skill, which updates the PR at [Phase 9](#phase-9-update-pr) once the force push has landed.

**Commit message rules:** `commits:create` owns and enforces the full set — subject (text after `type(scope): `) ≤ 50 and whole header ≤ 100 (commitlint `subject-max-length` / `header-max-length`), lowercase imperative no-period title, WHAT-not-WHY subject, body required for `feat`/`fix`/`refactor`, no issue/PR numbers, no AI `Co-authored-by` trailers. See [commits:create Rules](../autopilot-commits-create/SKILL.md#rules); this skill restages the changes and delegates the message creation to it.
...
This gate is why the PR update belongs here and not inside `commits:create` — the description must be built from commits the remote has actually received. `commits:create` therefore skips its own [Phase 5](../autopilot-commits-create/SKILL.md#phase-5-update-pr) when invoked from this skill.
...
3. If a PR exists, invoke `autopilot-pr-update` with `--autopilot`: the restructure the user just authorized is what changed the commits, so bringing the PR description into line with them is bookkeeping, not a decision — the flag is what makes the callee skip its own dialogs.
