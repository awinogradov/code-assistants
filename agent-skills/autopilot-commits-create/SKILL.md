---
name: autopilot-commits-create
description: >-
  Analyze staged changes and create conventional commits with intelligent
  grouping. Use when creating commits, or when invoked from other skills.
---
...
Read [`askuserquestion-contract.md`](../autopilot-shared-rules/references/askuserquestion-contract.md) and apply it to both dialogs. In this skill, `preview` belongs to the [validation failure dialog](#validation-failure-dialog) only — the strategy prompt is a plain choice and takes no `preview`.

## Phase 0: Preflight Check

Invoke `autopilot-preflight-check` with `mode: commits` from this conversation context. The skill verifies the current branch is appropriate for committing and warns if you are on `main`. If it outputs a "cancelled" message, stop immediately — do not proceed to [Phase 1](#phase-1-check-for-changes).
...
- **If agent recommends `singleCommitRecommended: false`:** the changeset is large enough to consider splitting. Evaluate whether the changes represent genuinely distinct areas. If a single coherent commit message can describe all changes, use single commit flow. Otherwise, ask the user:
  ...
  **Formatting Note:** Read [`askuserquestion-format.md`](../autopilot-shared-rules/references/askuserquestion-format.md) and apply it before composing the `question` parameter.
...
Pass = yes, yes, no. The binding conventions and examples are in [CONTRIBUTING.md § Commits](https://github.com/awinogradov/code-assistants/blob/main/CONTRIBUTING.md#commits).
...
- This skill was invoked from `autopilot-commits-restructure` — that skill runs its own PR update after its force push, which is the correct moment. Updating here would push a description built from commits the remote has not received yet.
...
3. If a PR exists, invoke `autopilot-pr-update` with `--autopilot`: the PR already exists and the commits are already made, so refreshing its title and description to match them is bookkeeping, not a decision — the flag is what makes the callee skip its own dialogs.
