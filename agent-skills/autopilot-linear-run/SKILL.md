---
name: autopilot-linear-run
description: >-
  Run any Linear issue end to end. Executes a valid stored plan verbatim when
  one exists; otherwise drafts, reviews, and implements a fresh plan without a
  human approval gate.
---

Run any Linear issue end to end. Prefer the durable plan [`linear:plan`](../autopilot-linear-plan/SKILL.md) stored in its description when that artifact validates; otherwise draft, review, and implement a fresh plan from the issue context.

**Difference from [`/autopilot:run`](../autopilot-run/SKILL.md):** `run` always drafts a plan. This skill first inspects the Linear issue for a checkable durable artifact. A valid stored plan is executed verbatim; missing or unusable stored-plan data selects the same autonomous planning pipeline as `run`. The choice is deterministic from the issue description, never inferred from conversation history or from a claim that another agent ran.
...
Identical to the `plan` skill — see [its Input resolution section](https://github.com/awinogradov/code-assistants/blob/main/claude-plugins/autopilot/skills/plan/SKILL.md#input-resolution) — narrowed to Linear issues, because this skill always reads the Linear issue before choosing its plan source.
...
**Linear MCP access:** Read [`linear-mcp-access.md`](../autopilot-shared-rules/references/linear-mcp-access.md) and apply its tool-resolution rule, using the bare tool name `get_issue`.
...
| **malformed**        | any required section from [the stored format](../autopilot-linear-plan/SKILL.md#the-stored-plan-format) is absent — name the specific omission in the message |
...
Identical to [`run`](../autopilot-run/SKILL.md#phase-2-preflight-verdict): read branch, worktree, `isStaleMerged`, and `baseAhead` from the Context Map, compare the issue id against the current branch name, and invoke `autopilot-preflight-check` only for a state the map does not cover. If it outputs "Planning cancelled", stop immediately.
...
Execute the shared pipeline in [pipeline.md](https://github.com/awinogradov/code-assistants/blob/main/claude-plugins/autopilot/skills/plan/references/pipeline.md) — draft, review and score, finalize — resolving your stack's deltas from [stack-deltas.md](https://github.com/awinogradov/code-assistants/blob/main/claude-plugins/autopilot/skills/plan/references/stack-deltas.md). Use the Common Instructions and plan-file header rule from [`run`](../autopilot-run/SKILL.md#common-instructions).
...
In `fresh-plan` mode, the finalized harness plan is the execution contract exactly as it is for [`run`](../autopilot-run/SKILL.md#phase-5-implement-and-proceed). This mode is autonomous and adds no approval prompt.

## Phase 6: Branch and run the autopilot chain

Follow [`run`'s Phase 4](../autopilot-run/SKILL.md#phase-4-embed-branch-creation-and-the-autopilot-chain) and [Phase 5](../autopilot-run/SKILL.md#phase-5-implement-and-proceed) without an approval gate.
...
- In `stored-plan` mode, do not modify the Linear description or synthesize a replacement plan file. Invoke `autopilot-branch-create` with the Linear issue using the body from [branch-blocks.md](https://github.com/awinogradov/code-assistants/blob/main/claude-plugins/autopilot/skills/plan/references/branch-blocks.md), then implement the frozen stored steps.
