---
name: autopilot-gather-context
description: >-
  Acquire all planning context in one parallel fan-out and emit a Context Map.
  Use when plan or run needs issue, standards, branch, and codebase context
  without loading raw documents into the parent conversation.
---
...
- **Scope** — `task` (the default), `broad`, or `primed`, selecting how [Phase 2](#phase-2-scope-the-codebase-pass) reads the snapshot. Optional: `plan` and `run` omit it and get `task`. `primed` additionally gates off one [Phase 1](#phase-1-fan-out) agent, so this input is a fan-out selector and not only a read strategy — see [`run-primed`](https://github.com/awinogradov/code-assistants/blob/main/claude-plugins/autopilot/skills/run-primed/SKILL.md), the only caller that passes it.
...
| Agent                                                              | When                       | Prompt                                                                                                     |
| ------------------------------------------------------------------ | -------------------------- | ---------------------------------------------------------------------------------------------------------- |
| [`digest-repo-standards`](../autopilot-digest-repo-standards/SKILL.md)   | Except `Scope: primed`     | `Repository root: [path]. Task summary: [summary].`                                                        |
| [`digest-branch-diff`](../autopilot-digest-branch-diff/SKILL.md)         | Always                     | `Repository root: [path]. Base ref: origin/main.`                                                          |
| [`resolve-issue-context`](../autopilot-resolve-issue-context/SKILL.md)   | Issue inputs               | Per that agent's Input section; pass `Auto-assign current user: true` for GitHub only                      |
| [`resolve-alert-context`](../autopilot-resolve-alert-context/SKILL.md)   | `code-scanning-alert` only | `Fetch alert context. Alert number: [n]. Repository: [owner/repo].`                                        |
| [`search-codebase-todos`](../autopilot-search-codebase-todos/SKILL.md)   | Issue inputs               | `Search for TODOs. Input type: [type]. Issue ID: [id].`                                                    |
| [`digest-session-history`](../autopilot-digest-session-history/SKILL.md) | Entire enabled (see below) | `Repository root: [path]. Task summary: [summary]. Relevant files: [paths the task text names, when any].` |

**Direct calls** in the same message:

- **Snapshot** — follow the ordered source chain in [`repomix-snapshot.md`](../autopilot-shared-rules/references/repomix-snapshot.md); this skill passes no `includePatterns`. Record the selected source, and the returned `outputId` when the repomix tier was selected.
...
When you write the Context Map, apply the reference-formatting rules in [`reference-formatting.md`](../autopilot-shared-rules/references/reference-formatting.md) (RFC-0001, read it first) to every reference it contains — link files, docs, skills, agents, and sections, and never leave a reference as bare text.
