---
name: autopilot-digest-branch-diff
description: >-
  Summarize a branch's commits and diff against main, and detect a stale-merged
  branch whose work already landed upstream. Use when planning skills need the
  in-flight change set without an unbounded diff in parent context.
---
> Derived from the autopilot `digest-branch-diff` subagent. Where subagents are unavailable, run this task inline and treat its structured output block as the result handed back to the invoking workflow.
