...
Read [`repomix-snapshot.md`](../autopilot-shared-rules/references/repomix-snapshot.md) for the ordered context-acquisition chain; this skill passes the review-scoped `includePatterns` (repomix tier only) shown below.
...
Only provide `updatedReviewComment` if `updatedVerdict` is non-null. Follow the same format as the original review body (see the pr:review skill for format rules) — including `CHECK-` rule codes rendered exactly per the [`pr:review` skill's §2.5](../autopilot-pr-review/SKILL.md#25-rule-codes). Do not read agent files.
