---
name: autopilot-digest-repo-standards
description: >-
  Read a repository's own README, docs/, rfc/, principles/, and CLAUDE.md and
  return a bounded standards digest. Use when planning skills need the project's
  conventions without loading the full documents into parent context.
---
> Derived from the autopilot `digest-repo-standards` subagent. Where subagents are unavailable, run this task inline and treat its structured output block as the result handed back to the invoking workflow.
