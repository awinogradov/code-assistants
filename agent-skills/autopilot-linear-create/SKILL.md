---
name: autopilot-linear-create
description: >-
  Create a Linear issue with a structured body (Context, What, Why, Scope,
  Solution) and wizard-selected status, label, and assignee via the Linear MCP.
  Use when filing a Linear ticket on a linear-tracked project.
---
...
**Linear MCP access:** Read [`linear-mcp-access.md`](../autopilot-shared-rules/references/linear-mcp-access.md) and apply its tool-resolution rule, using the bare tool names `save_issue`, `list_issue_statuses`, `list_issue_labels`.
...
1. Acquire codebase context once by following the ordered source chain in [`repomix-snapshot.md`](../autopilot-shared-rules/references/repomix-snapshot.md); this skill passes no `includePatterns`. Record the selected source, and the `outputId` when the repomix tier was selected.
