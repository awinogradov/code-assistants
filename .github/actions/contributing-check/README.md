...
Dependency-bot PRs (author `dependabot[bot]` or `renovate[bot]`) skip every step — Dependabot's machine-generated `dependabot/...` branches and update titles, and Renovate's `chore(deps): …` commit subjects, do not follow the human conventions. Each step is gated on the PR author, so the "Validate PR" job still reports success (green) rather than a stuck or failed required check.
