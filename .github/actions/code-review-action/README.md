...
[![GitHub Release](https://img.shields.io/badge/release-v1.4.1-blue)](https://github.com/awinogradov/code-assistants/releases/latest)
...
| `model`                | no       | `claude-sonnet-4-6`      | Claude model to use (the default tier; also the ceiling when `model_tiering` is on).                                                                                          |
| `model_tiering`        | no       | `off`                    | Route small, low-risk PRs (docs/test/config/style only, small churn, no security or CI paths) to `tier_model`. `auto` enables it; `off` always uses `model`.                  |
| `tier_model`           | no       | `claude-haiku-4-5`       | Cheaper model used for small, low-risk PRs when `model_tiering` is `auto`.                                                                                                    |
