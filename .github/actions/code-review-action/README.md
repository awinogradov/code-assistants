...
[![GitHub Release](https://img.shields.io/badge/release-v11.0.0-blue)](https://github.com/awinogradov/code-assistants/releases/latest)
...
| `generate_review_tips` | no       | `false`                  | Opt-in: on the ~5% review-tip roll, generate the tip from bounded consumer context (this repo's CONTRIBUTING.md, docs listing, changed files) via one small model call, falling back to the static pool on any failure. See [Random review tip](#random-review-tip). |
...
The version-1 contract lives in [`reviewContextBundle.ts`](src/reviewContextBundle.ts): every bound (file entries, diff bytes, threads, review bodies) is an exported constant, every truncation is flagged in the payload, and each fallible section is either complete data or an explicit `{ available: false, reason }` — rate limits (`rate-limited`) are distinguished from endpoint failures. Re-reviews carry an explicit delta from the **substantive anchor** — the reviewer's latest `APPROVED`/`CHANGES_REQUESTED` review, or a `COMMENTED` one with a non-empty top-level body; an empty `COMMENTED` review created by replying inside an inline thread never advances the anchor (issue [#622](https://github.com/awinogradov/code-assistants/issues/622)). A force-pushed or diverged ref degrades the delta explicitly rather than pretending nothing changed. The diff is streamed to a separate file with the byte cap enforced mid-stream, never buffered.

The builder acquires raw facts only — round **policy** (full vs incremental vs skip, and the review surface for each) is owned by the `pr-review` skill's §1.2 state machine, which routes the delta's reason strings: `compare-status-identical` skips, `compare-status-diverged`/`compare-status-behind`/`ref-missing` force a full review. [`reviewRoundContract.test.ts`](src/reviewRoundContract.test.ts) pins the skill prose and the builder's `isSubstantiveReview` predicate to each other — editors of the skill's §1.2 wording should expect that test to fail until both sides agree. De-poisoning the anchor shifts telemetry: expect fewer `compare-status-identical` deltas and more `ahead` ones after upgrading. On publication, [`submitReview.ts`](src/submitReview.ts) binds the created review to the expected head SHA via `commit_id`, so a head that moves after review cannot claim the verdict.

The skill classifies the round before reading a patch or acquiring supporting context. An already-reviewed head exits immediately; an incremental round reads the compare patch, and reads the full diff only on fallback. The builder still writes the full-diff artifact outside model context. Review issue lookups use the bundled GitHub and Linear helpers directly; the action grants only their trusted installed paths, with `--read-only` required for GitHub. Issue payload truncation and lookup errors stay visible, and TODO searches retain at most 20 matches.
...
Roughly 5% of reviews carry one extra rotating `> [!TIP]` — a usage or convention tip from a curated pool, never repeated within the same PR (a hidden per-tip marker in prior bot reviews tracks what was shown). Tips never appear on clean approvals, never affect duplicate-review detection, and fail open: if the prior-review lookup fails, the review posts untipped. The static pool and the ~5% rate are hardcoded — consumers tracking `@main` receive tips on merge.

Set `generate_review_tips: "true"` to instead generate that tip from bounded consumer context (this repo's CONTRIBUTING.md, its docs listing, and the PR's changed files) via one small `continue-on-error` model call, framed as untrusted data and validated on the way out; the static pool remains the fallback, so generation never blocks or degrades the review.

See [Random review tip](../../../docs/03-code-review-run-summary.md#random-review-tip) for the marker contract and data flow, and [Consumer-generated tips](../../../docs/03-code-review-run-summary.md#consumer-generated-tips-opt-in) for the generation path.
