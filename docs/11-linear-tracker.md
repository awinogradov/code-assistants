...
- **The prior description moves into a `+++ Original task +++` collapsible** on the first store, using the same fence [`linear:create`](../claude-plugins/autopilot/skills/linear:create/SKILL.md) uses for its original-prompt preamble. A ticket created by that skill therefore ends up with one collapsible nested inside another. This is accepted deliberately; Linear's [GraphQL markdown documentation](https://linear.app/developers/graphql) describes the fence without specifying nesting behaviour.
