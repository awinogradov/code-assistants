...
## Planning and Linear follow-up

[Gather-context](../claude-plugins/autopilot/skills/gather-context/SKILL.md) resolves intent before dependent research and reuses same-invocation ticket data. [Explicit briefs](./15-run-primed-skill.md) and [stored Linear evidence](./16-linear-plan-skill.md) narrow investigation while preserving current-code and standards checks. [Explore](./14-explore-skill.md) uses dependency-based refresh and conditional diagrams. Shared planning rules are read once per context holder: this shared-source extraction avoids loading the plan orchestrator in run and Linear callers. Conditional storage references keep unrelated procedures out of other inputs, and progress tracks substantive outcomes rather than parsing or draft/finalize bookkeeping.

[Linear creation](../claude-plugins/autopilot/skills/linear-create/SKILL.md) gathers independent metadata concurrently and combines choices where supported, preserving explicit user selections and final preview. These changes reduce prescribed work; compare representative total-agent tokens, cache usage, tool calls, elapsed time, and rework before claiming measured performance improvements.
