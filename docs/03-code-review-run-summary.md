...
**Review runs carry an optional `contextBuilder` block.** When the review ran with a startup context bundle (issue #605), the summary gains a `context_builder` object — builder duration, source request count, bundle and diff bytes, truncation, cache use, plus the session-side `bundle-followup:` count and `bundle-fallback:` status counted by `runClaude.ts` from the transcript — rendered as an extra **Context bundle** table row and a camelCase `contextBuilder` key in the data comment. The field is optional end to end: react/preflight runs, pre-bundle action versions, and a crashed builder simply omit it, and the monitor's schema treats it as optional, so old footers keep parsing.
...
The static pool and the ~5% rate are hardcoded — no action input configures them. Consumers tracking the action `@main` receive tips on merge; disabling the static pool means reverting the feature commit upstream. Tip links are absolute URLs into this repository whose paths must exist in-tree, guarded by a `reviewTip.test.ts` case so a docs move cannot silently 404 a shipped tip.

### Consumer-generated tips (opt-in)

Static tips are universal by construction — every consumer sees the same upstream-authored texts, and none can reference the consuming repo's own conventions or the PR at hand. Setting the `generate_review_tips` input to `"true"` layers an optional generation path on top of the same roll ([issue 395](https://github.com/awinogradov/code-assistants/issues/395)): when a tip is warranted, a small model call — the existing `runClaude.ts` engine as a separate `continue-on-error` step (`mode: tip`), so cost comes from the SDK's `total_cost_usd` — writes the tip from bounded consumer context instead of picking from the pool. The static pool stays the fallback, and generation never blocks or degrades the review.

Three new steps sit between the review and submit steps, mirroring the preflight triad:

- **`prepareReviewTip.ts` (step `prep_tip`)** owns the roll on this path: it excludes tips the PR already saw, picks one against the 5% gate, and — unless the review is a clean approval or the pool is exhausted — assembles the bounded consumer context (`CONTRIBUTING.md` capped, the `docs/` listing, and the PR's changed-file summary), builds an untrusted-data prompt, and writes it to a `$RUNNER_TEMP` file. It emits `generate`, `tip_prompt_file`, and `fallback_tip` (the selected static tip as JSON).
- **`runClaude.ts` (step `gen_tip`)** is the same engine the review uses, run with `CLAUDE_RUN_MODE=tip` and an `{id, text}` JSON schema. It returns the candidate as `structured_output` and its metrics as `run_summary`. The step is `continue-on-error`.
- **`submitReview.ts` (step `submit`)** consumes the decision when `TIP_GENERATION` is `"true"`: it validates the candidate and renders it with a `gen-<id>` marker, falling back to the static `fallback_tip` on any failure. When `TIP_GENERATION` is not `"true"` (every consumer that has not opted in) submit rolls inline against the static pool exactly as before — the default path is unchanged.

Two safeguards keep generation from breaking the tip contracts or the review:

- **Untrusted context, both sides.** The prompt frames `CONTRIBUTING.md`, the docs listing, and the changed files as untrusted DATA between `<<<CONTEXT>>>` and `<<<END>>>` markers and forbids following any instruction inside them (the same posture as the preflight-explain prompt). On the output side, `reviewTipGeneration.ts` validates the candidate independently — collapse to one line, strip backticks and angle brackets so no HTML tag, comment, or run-summary marker can survive, reject any candidate carrying a URL or markdown link, cap the length, and namespace the id `gen-<slug>` — so `extractShownTipIds`, `stripReviewTips`, and `reviewDedupKey` (which match the `[a-z0-9-]` marker charset) keep working unchanged.
- **Fail-open and cost-bounded.** The 5% gate bounds cost to roughly one small model call per twenty reviews. No key, a model error, or an invalid candidate falls back to the static pool; a `prep_tip` glitch drops to no tip. The generation call's cost is reported via the SDK run summary in the workflow run log; because the review comment carries a single run-summary footer (the review's), the generation cost is not aggregated by the cost monitor's posted-footer scan.
