...
**`claude-plugin` npm opt-in.** A `claude-plugin` monorepo member publishes to npm exactly when its `package.json` does **not** declare `"private": true` — dropping `private` (plus normal npm publication metadata: a scoped `name`, `files`, `publishConfig.access`) is the opt-in, so existing private plugins keep the historical no-npm behavior. The action supports two npm auth paths: the `npm_token` input, or trusted publishing (OIDC) when the calling workflow can obtain an OIDC token (`id-token: write` granted and npm CLI ≥ 11.5.1). The stock [release-publish workflow](../.github/workflows/release-publish.yml) is `pull_request_target`-triggered, and GitHub does not issue OIDC tokens to such runs (see [community discussion #137761](https://github.com/orgs/community/discussions/137761)) — so it passes `NPM_TOKEN`, which its consumers must configure. A brand-new package also needs a one-time token-based bootstrap publish before a registry-side trusted publisher could be configured at all.
