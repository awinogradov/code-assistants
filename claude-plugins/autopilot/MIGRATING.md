...
- `/autopilot:pr-monitor` no longer polls from inside the model. It launches the bundled watcher `lib/github/watch-pr.ts`, which waits in its own process and returns one bounded JSON event, and the skill acts on that event and relaunches with `--ack <event-id>`. The skill's `allowed-tools` drops `Bash(sleep *)`. Behaviour that callers depend on is unchanged — the default `READY_FOR_REVIEW` hand-off, `--wait-for-approval`, the Conflict Sweep and its run-scoped cap of 2, foreground remediation versus background reporting — but two things are new: a `BLOCKED` exit for operational failures (auth loss, exhausted retries, an unreadable required-check policy, a head that never registers checks, a deadline or cancellation), and watcher state under `<git common dir>/autopilot/pr-watch/`. A runtime that cannot notify the session when a background process exits is no longer supported for monitoring: the skill reports that and stops rather than falling back to a timer.
