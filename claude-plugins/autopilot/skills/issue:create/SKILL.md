...
**Fallback:** if the repomix MCP server is unavailable, use `Grep` and `Read` directly on the repository (same files, smaller scope). Do not block the skill on MCP availability.
