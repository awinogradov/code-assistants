...
1. Acquire the codebase snapshot once by following [`repomix-snapshot.md`](../shared-rules/references/repomix-snapshot.md). This skill reasons over the whole tree, so it passes no `includePatterns`. Store the returned `outputId`.
