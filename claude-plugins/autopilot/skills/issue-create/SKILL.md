...
1. Acquire codebase context once by following the ordered source chain in [`repomix-snapshot.md`](../shared-rules/references/repomix-snapshot.md). This skill reasons over the whole tree, so it passes no `includePatterns`. Record the selected source, and the returned `outputId` when the repomix tier was selected.
